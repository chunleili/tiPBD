...
class GGUI:
    def __init__(self) -> None:
        ...
        self.cam_pos = meta.get_common("camera_pos", default=[0.5, 0.5, 2])
        self.cam_lookat = meta.get_common("camera_lookat", default=[0.5, 0.5, 1])
        ...
        meta.mesh_uniform_color = tuple(
            meta.get_common("mesh_uniform_color", default=self.uniform_color, no_warning=True)
        )
        meta.particle_uniform_color = tuple(
            meta.get_common("particle_uniform_color", default=self.uniform_color, no_warning=True)
        )
    ...
    def update(self, pos_show, indices_show=None):
        ...
        if indices_show is not None and meta.show_mesh:
            self.scene.mesh(
                pos_show, indices=indices_show, color=meta.mesh_uniform_color, show_wireframe=meta.show_wireframe
            )
        if meta.show_particles:
            if meta.use_selector:
                self.scene.particles(
                    pos_show, radius=meta.particle_radius_show, per_vertex_color=meta.particle_per_vertex_color
                )
            else:
        ...
        if meta.show_auxiliary_meshes:
            self.scene.mesh(
                self.ground, indices=self.ground_indices, color=(0.5, 0.5, 0.5), show_wireframe=meta.show_wireframe
            )
            self.scene.mesh(
                self.coord, indices=self.coord_indices, color=(0.5, 0, 0), show_wireframe=meta.show_wireframe
            )
...
def visualize(
    par_pos=None,
    particle_radius_show=0.01,
    mesh_pos=None,
    mesh_indices=None,
    ti_init=False,
    background_color=(1, 1, 1),
    show_widget=False,
    par_color=(0.1229, 0.2254, 0.7207),
):
    import numpy as np
...
def vis_sdf(grid, provide_render=True):
    ...
    if provide_render:
        ...
        cam_pos = [-0.71, 1.78, 1.77]
        cam_lookat = [-0.159, -1.1578, 1.213]
    ...
    def occ():
        for i, j, k in grid:
            if grid[i, j, k] < threshold:
                ...
                for l, val in ti.static(
                    enumerate([0, 1, 0, 2, 1, 3, 2, 3, 4, 5, 4, 6, 5, 7, 6, 7, 0, 4, 1, 5, 2, 6, 3, 7])
                ):
                    indices[i, j, k, l] = val
