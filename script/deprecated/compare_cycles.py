...
def amg_cg_solve(levels, b, x0=None, tol=1e-5, maxiter=100, cycle='V'):
    x = x0.copy()
    A = levels[0].A
...
def V_cycle(levels,lvl,x,b):
    ...
    presmoother(A,x,b)
    toc = perf_counter()
    print(f"V level {lvl} presmoother time: {toc-tic}")
    residual = b - A @ x
...
def W_cycle(levels,lvl,x,b):
    A = levels[lvl].A
    tic = perf_counter()
    presmoother(A,x,b)
    toc = perf_counter()
    print(f"W level {lvl} presmoother time: {toc-tic}")
    residual = b - A @ x
    coarse_b = levels[lvl].R @ residual
    coarse_x = np.zeros_like(coarse_b)
    if lvl == len(levels)-2:
        ...
        W_cycle(levels, lvl + 1, coarse_x, coarse_b)
        W_cycle(levels, lvl + 1, coarse_x, coarse_b)
    x += levels[lvl].P @ coarse_x
    tic = perf_counter()
    postsmoother(A, x, b)
    toc = perf_counter()
    print(f"W level {lvl} postsmoother time: {toc-tic}")
...
def compare_adaptive_SA(postfix='F10-0'):
    ...
    allres.append(Residual('V', residuals, toc-tic))
    ...
    tic = perf_counter()
    x0 = np.zeros_like(b)
    x,residuals = amg_cg_solve(levels, b, x0=x0.copy(), maxiter=maxiter, tol=1e-6, cycle='W')
    toc = perf_counter()
    print("solve phase:", toc-tic)
    allres.append(Residual('W', residuals, toc-tic))
    conv = calc_conv(residuals)
    print("conv:", conv)
