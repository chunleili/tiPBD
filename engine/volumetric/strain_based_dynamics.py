...
def project_constraints():
    ...
    stretch_stiffness = vec3(1.0)
    shear_stiffness = vec3(1.0)
    for t in tet:
        p0 = tet[t][0]
        p1 = tet[t][1]
        p2 = tet[t][2]
...
@ti.func
def solve_StrainTetraConstraint(
    x0,
    x1,
    x2,
    x3,
    ...
    for i in range(3):
        for j in ti.static(range(3)):
            if j <= i:
                ...
                lam = (
                    inv_mass0 * d[0].norm_sqr()
                    + inv_mass1 * d[1].norm_sqr()
                    + inv_mass2 * d[2].norm_sqr()
                    + inv_mass3 * d[3].norm_sqr()
                )

                if i == j:
                    if normalize_stretch:
                        s = ti.sqrt(Sij)
                        lam = 2.0 * s * (s - 1.0) / lam * stretch_stiffness[i]
                    else:
                        lam = (Sij - 1.0) / lam * stretch_stiffness[i]
                else:
