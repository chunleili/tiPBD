...
class SDF:
    ...
    This implementation tackles the boundary issue with seperate disscussion.
    It use ndrange to loop, which will destroy the sparsity of the field, but avoid if statements in the for loops.
    """
    ...
    def compute_gradient_2d(self: ti.template(), dx: ti.f32, dy: ti.f32):
        ...
        for i in range(1, self.shape[0] - 1):
            self.grad[i, 0] = ti.Vector(
                [(self.val[i + 1, 0] - self.val[i - 1, 0] / dx), (self.val[i, 1] - self.val[i, 0]) / dy]
            )
        ...
        for j in range(1, self.shape[1] - 1):
            self.grad[0, j] = ti.Vector(
                [(self.val[1, j] - self.val[0, j] / dx), (self.val[0, j + 1] - self.val[0, j - 1]) / dy * 0.5]
            )
