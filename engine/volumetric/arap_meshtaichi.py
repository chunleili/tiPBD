...
class Model:
    def __init__(self, geometry_file, direct_import_faces=True):
        ...
        self.mesh.cells.place(
            {"inv_vol": ti.f32, "B": ti.math.mat3, "lagrangian": ti.f32, "fem_constraint": ti.f32, "alpha": ti.f32}
        )
...
class ARAP:
    ...
    def compute_denorminator(self, c, g0, g1, g2, g3):
        p0, p1, p2, p3 = c.verts[0], c.verts[1], c.verts[2], c.verts[3]
        res = (
            p0.inv_mass * g0.norm_sqr()
            + p1.inv_mass * g1.norm_sqr()
            + p2.inv_mass * g2.norm_sqr()
            + p3.inv_mass * g3.norm_sqr()
        )
        return res
...
def log_energy(model, write_energy_to_file=False, interval=100):
    if meta.frame % interval == 0:
        logging.info(
            f"frame: {meta.frame} potential: {model.potential_energy[None]:.3e} inertial: {model.inertial_energy[None]:.3e} total: {model.total_energy[None]:.3e}"
        )
