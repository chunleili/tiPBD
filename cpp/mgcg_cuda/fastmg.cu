...
/* -------------------------------------------------------------------------- */
    ...
    void  FastMG::setup(std::vector<SpMatData*> Ps) {
        for(int i=0; i<Ps.size(); i++)
        {
            set_P(i, Ps[i]->data.data(), Ps[i]->data.size(),
                Ps[i]->indices.data(), Ps[i]->indices.size(),
                Ps[i]->indptr.data(), Ps[i]->indptr.size(),
                Ps[i]->nrows(), Ps[i]->ncols(), Ps[i]->nnz());
        }
    }
