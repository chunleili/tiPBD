...
parser.add_argument("-solver_type", type=str, default='GMG', help='"AMG", "GS", "XPBD","GMG"')
...
# def define_prolongator_v1():
#     from scipy.sparse import csr_matrix
#     P = csr_matrix((NE, NE/16), dtype=np.float32)
#     # for each row, we define the diag is 1.0(which is i), the first ring neighbors are 0.75, and second-ring neighbor are 0.5, and the third ring neighbors are 0.25.
#     for i in range(NE):
#         P[i, i] = 1.0
#         for j in range(num_adjacent_edge[i]):
#             ia = adjacent_edge[i, j]
#             P[i, ia] = 0.75
#             for k in range(num_adjacent_edge[ia]):
#                 ib = adjacent_edge[ia, k]
#                 if ib == i:
#                     continue
#                 P[i, ib] = 0.5
#                 for l in range(num_adjacent_edge[ib]):
#                     ic = adjacent_edge[ib, l]
#                     if ic == i:
#                         continue
#                     P[i, ic] = 0.25


def define_prolongator_v2():
    from scipy.sparse import coo_matrix, lil_matrix
    # P = lil_matrix((NV, int(NV/16)), dtype=np.float32)
    P = np.zeros((NV, int(NV/16)), dtype=np.float32)
    #       o---o---o---o---o
    #       |   |   |   |   |
    #  .25  o---o---o---o---o
    #       |   |   |   |   |
    ...
    #  1.0  o---o---o---o---o
    #     1.0 .75 .50 .25
    # the (0,0) is 1.0, the (0,1) is 0.75, the (0,2) is 0.5, the (0,3) is 0.25
    #                   the (1,0) is 0.75, the (2,0) is 0.5, the (3,0) is 0.25
    #                   the (1,1) is 0.75^2 = 0.5625, the (1,2) is 0.75*0.5 = 0.375, the (1,3) is 0.75*0.25 = 0.1875
    for i in range(NV):
        P[i, i] = 1.0
        upper = i + (N + 1)
        lower = i - (N + 1)
