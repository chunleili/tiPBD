...
@ti.kernel
def solve_subspace_constraints_xpbd_kernel(
    labels: ti.template(),
    ...
    for i in range(ist.NE):
        ...
        dis = pos[idx0] - pos[idx1]
        constraint = dis.norm() - rest_len[i]
        numerator[i] = -(constraint + lagrangian[i] * ist.alpha)
        denominator[i] = invM0 + invM1 + ist.alpha

    # lumped y_jprime
    for i in range(ist.new_M):
...
def solve_subspace_constraints_xpbd():
    NE = ist.NE
    new_M = ist.new_M
    labels = ti.field(dtype=int, shape=(NE))
    ...
    labels.from_numpy(ist.labels)

    # presmooth
    solve_distance_constraints_xpbd(ist.dual_residual, ist.inv_mass, ist.edge, ist.rest_len, ist.lagrangian, ist.dpos, ist.pos)
    # subspace solve
    solve_subspace_constraints_xpbd_kernel(labels, numerator, denominator, numerator_lumped, denominator_lumped, y_jprime, ist.dLambda, ist.inv_mass, ist.edge, ist.rest_len, ist.lagrangian, ist.dpos, ist.pos)
    # postsmooth
    solve_distance_constraints_xpbd(ist.dual_residual, ist.inv_mass, ist.edge, ist.rest_len, ist.lagrangian, ist.dpos, ist.pos)

    

def set_labels_from_P(labels, P):
    assert P.dtype == bool
    assert P.format=="csc"
    for j in range(P.indptr.shape[0]-1):
        for i in range(P.indptr[j], P.indptr[j+1]):
            labels[P.indices[i]] = j
    return labels

# lumped XPBD
def substep_xpbd_lumped():
    if should_setup():
        A = fill_A_csr_ti(ist)
        args.filter_P = "01"
        Ps = build_Ps(A,args, extlib)
        ist.P = Ps[0].astype(bool)
        ist.new_M = Ps[0].shape[1]
        ist.labels = np.zeros(shape=ist.P.shape[0], dtype=int)
        ist.P = ist.P.tocsc()
        ist.labels = set_labels_from_P(ist.labels, ist.P)
    ...
    for ist.ite in range(args.maxiter):
        ...
        reset_dpos(ist.dpos)
        solve_subspace_constraints_xpbd()
