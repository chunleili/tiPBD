...
std::vector<double> easy_calc_rbm(const std::vector<double> &coo) {
    std::vector<double> B;
    rigid_body_modes(3, coo, B);
    return B;
}
...
// coo in xyz xyz xyz format, B in 123456 123456... format
//Bsize: n * 6
extern "C" DLLEXPORT void fastmg_calc_rbm(const double *coo, const int ncoo, double *resB) {
    // force to double
    std::vector<double> B_vec;
    std::vector<double> coo_vec(coo, coo + ncoo);
    rigid_body_modes(3, coo_vec, B_vec);
    std::copy(B_vec.begin(), B_vec.end(), resB);
}