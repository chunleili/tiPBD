...
pos_np, edge_np, tri_np = read_tri_cloth_obj("./data/model/tri_cloth/tri_cloth.obj")
# pos_np, edge_np, tri_np = read_tri_cloth("./data/model/tri_cloth/tri_cloth") # .node .edge .face
NE = edge_np.shape[0]
NV = pos_np.shape[0]
NT = tri_np.shape[0] // 3
...
def init_rest_len():
    for i in rest_len:
        rest_len[i] = (pos[edge[i][0]] - pos[edge[i][1]]).norm()


@ti.kernel
def init_inv_mass(fixed_point1: int, fixed_point2:int):
    for i in inv_mass:
        inv_mass[i] = 1.0
    inv_mass[fixed_point1] = 0.0
    inv_mass[fixed_point2] = 0.0
...
def transfer_back_to_pos_mfree(x):
    ...
    update_pos(inv_mass, dpos, pos)
    # collision(pos) # will get bug because of collision
