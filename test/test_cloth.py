...
def test_cloth_xpbd_gpu_calc_strain():
    run_args = ["python", "engine/cloth/cloth3d.py","-N=64", "-solver_type=XPBD","-arch=gpu", "-end_frame=1", "-calc_strain=1", "-export_strain=1"]
    ret = subprocess.check_call(run_args)
    assert ret == 0


def test_cloth_xpbd_gpu_calc_energy():
    run_args = ["python", "engine/cloth/cloth3d.py","-N=64", "-solver_type=XPBD","-arch=gpu", "-end_frame=1", "-calc_energy=1"]
    ret = subprocess.check_call(run_args)
    assert ret == 0


def test_cloth_amg_cuda_calc_strain():
    run_args = ["python", "engine/cloth/cloth3d.py","-N=64", "-use_cuda=1", "-solver_type=AMG", "-end_frame=1", "-export_strain=1", "-calc_strain=1"]
    ret = subprocess.check_call(run_args)
    assert ret == 0

def test_cloth_amg_cuda_calc_energy():
    run_args = ["python", "engine/cloth/cloth3d.py","-N=64", "-use_cuda=1", "-solver_type=AMG", "-end_frame=1", "-calc_energy=1"]
    ret = subprocess.check_call(run_args)
    assert ret == 0