...
DIM = 3
model_pos, model_inx, model_tri = read_tet_mesh("bunny1000_2000/bunny1000_dilate_new")
...
P = sio.mmread("bunny1000_2000/P.mtx")
fine_pos_init, fine_tet_idx, fine_tri_idx = read_tet_mesh(
    "bunny1000_2000/bunny2000")
fine_pos_ti = ti.Vector.field(DIM, float, fine_pos_init.shape[0])
...
def computeGradientVector():
    for i in range(NT):
        ...
        alpha = inv_h2 * inv_lame * invVol[i]
...
if __name__ == "__main__":
    ...
    while window.running:
        ...
        if window.is_pressed("c"):
            show_coarse = not show_coarse

        if window.is_pressed("f"):
            show_fine = not show_fine
