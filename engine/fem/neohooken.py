...
class Meta:
    def __init__(self):
        ...
        self.lame_lambda = self.youngs_modulus * self.poissons_ratio / (1+self.poissons_ratio) / (1-2*self.poissons_ratio)
...
class NeoHooken():
    ...
    def substep(self):
        ...
        for ite in range(meta.max_iter):
            self.project_constraints(self.tet_indices, self.B, self.inv_mass, self.lagrangian_H, self.lagrangian_D, self.alpha_tilde_H, self.alpha_tilde_D, self.pos)
    ...
    @ti.kernel
    def pre_solve(self, dt: ti.f32, pos: ti.template(), predic_pos: ti.template(),
                prev_pos: ti.template(), vel: ti.template()):
    ...
    @ti.kernel
    def post_solve(self, dt: ti.f32, pos: ti.template(), old_pos: ti.template(),
                    vel: ti.template()):
        # update velocity
        for i in pos:
